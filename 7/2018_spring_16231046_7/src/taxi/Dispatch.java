	...
	public void run() {
		do {
			...
			for (int i = 0; i < rq.getNum(); i++) {
				...
				if (!rq.getuse(i) && now_time <= rq.gettime(i) + 3000) {
			...
			for (int i = 0; i < rq.getNum(); i++) {
				...
				if ((!rq.getuse(i)) && (now_time > rq.gettime(i) + 3000)) {
					...
					if(ava_car.ava_num==0) {
						Main.data().println("----------------------------------------------------------------------------");
						Main.data().println("乘客请求内容----发出时刻："+rq.gettime(i)+" 请求坐标：("+src.x+","+src.y+")"+" 目的地坐标：("+rq.getdstp(i).x+","+rq.getdstp(i).y+")");
					...
					for(int j = 0 ; j < ava_car.ava_num;j++) {
						...
						Main.data().println("出租车信息----车辆编号："+car_t[j].getid()+" 所在位置：("+car_t[j].getpos().x+","+car_t[j].getpos().y+")"+" 车辆状态："+car_t[j].getstate()+" 车辆信用信息："+car_t[j].getcredit());
					}
					/////////           排序                        /////////////////
					Arrays.sort(car_t, new Comparator<Car>() {
						...
						public int compare(Car o1, Car o2) {
							if(o1.getcredit() > o2.getcredit()) {//信用高的排前
								...
								if( (ma.mindis(o1.ptn(o1.getpos()), o1.ptn(src))) < (ma.mindis(o2.ptn(o2.getpos()), o2.ptn(src))) ) {
									return -1;
								}else if ( (ma.mindis(o1.ptn(o1.getpos()), o1.ptn(src))) > (ma.mindis(o2.ptn(o2.getpos()), o2.ptn(src))) ) {
									return 1;
								}else {
									return 0;
								}
