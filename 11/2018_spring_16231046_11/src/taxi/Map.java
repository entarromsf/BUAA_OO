...
public class Map {
	...
	public boolean repOK(){
		/*@ REQUIRES: None;
		@ MODIFIES: None;
		@ Effects: 检查合法性的工作在readmap函数里做了，因此这里恒为true.
		*/
		return true;
	}
		...
		for (int i = 0; i < 80; i++) {
			for (int j = 0; j < 80; j++) {
				if (mapinfo[i][j] == 1 || mapinfo[i][j] == 3) {
					...
					graph2[i * 80 + j][i * 80 + j + 1] = 1024;
					graph[i * 80 + j + 1][i * 80 + j] = 1024;
					graph2[i * 80 + j + 1][i * 80 + j] = 1024;
				}
				if (mapinfo[i][j] == 2 || mapinfo[i][j] == 3) {
					graph[i * 80 + j][(i + 1) * 80 + j] = 1024;
					graph2[i * 80 + j][(i + 1) * 80 + j] = 1024;
					graph[(i + 1) * 80 + j][i * 80 + j] = 1024;
					graph2[(i + 1) * 80 + j][i * 80 + j] = 1024;
				}
			}
		}
	}
	...
	public void setFlow(int x1,int y1,int x2,int y2,int flow) {
		...
		if(graph[t1][t2] >= 1024 && graph[t1][t2] < gv.MAXNUM) {
			...
			graph2[t1][t2] = 1024 + flow;
			graph[t2][t1] = 1024 + flow;
			graph2[t2][t1] = 1024 + flow;
		}else {
...
//    }
	...
	public int[] pointbfs2(int root, int aim) {
		/*
         * @REQUIRES:
         *      0<=root<=6399;
         *      0<=aim<=6399;
		...
		for (int i = 1; i < 6400; i++) {
			...
			for(int j = 0; j < 6400; j++) {
				int tmp = (graph2[k][j] >= gv.MAXNUM ? gv.MAXNUM : (min + graph2[k][j]));
				if (!flag[j] && (tmp < dist[j])) {
					dist[j] = tmp;
					prev[j] = k;
				}
