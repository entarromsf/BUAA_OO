...
public class Requestadd extends Thread {
	...
	public Requestadd(RequestQue req_t, Map map_t, Car[] cars_t) {
		 ...
		 * 
		 * @ EFFECTS: req = req_t; ma = map_t; cars = cars_t;
		 */
		req = req_t;
		ma = map_t;
		cars = cars_t;
	...
	public void run() {
		...
		try {
			do {
				...
				Pattern t = Pattern.compile("^taxi");
				Matcher m = p.matcher(str_in);
				Matcher m1 = t.matcher(str_in);
				...
				if (m1.find()) {
					String temp = str_in.replaceAll("taxi", "");
					int tnum = Integer.parseInt(temp);
					if (tnum < 30) {
						cars[tnum].Infoprint();
					}
				} else if (loadsign && initinput) {// 文件输入
